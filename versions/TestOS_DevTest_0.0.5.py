...
version = "DevTest 0.0.5"  # Обновлённая версия!
DATA_ROOT = "D:\\TestOS_data"

# Проверяем доступность звуковых функций
try:
    import winsound
    sound_enabled = True
except ImportError:
...
def generate_sine_wave(frequency, duration, sample_rate=44100):
    """Генерирует синусоидальную звуковую волну"""
    samples = []
...
def play_sound(frequency, duration):
    """Воспроизводит звук с заданной частотой и длительностью"""
    try:
        # Проверяем, не в IDLE ли мы
        if 'idlelib.run' in sys.modules:
            return
            
        # Используем встроенную функцию Beep из winsound
        winsound.Beep(frequency, int(duration * 1000))
    except:
        # Если что-то пошло не так, просто игнорируем звук
        pass

def show_bsod(error_code="0x00000001"):
    """Функция для отображения синего экрана смерти (BSOD)"""
    try:
        # Воспроизводим звук BSOD (20000 Гц, 1000 мс)
        play_sound(2000, 2.0)
        ...
        # Воспроизводим звук запуска системы (300 Гц, 800 мс)
        play_sound(300, 0.8)
        os.system('cls' if os.name == 'nt' else 'clear')
...
Новые возможности:
- Звуковые эффекты (BSOD и запуск системы)
(только в консоли)
\n"""
    
    def __init__(self):
        super().__init__()
        # Воспроизводим звук запуска системы при старте
        play_sound(300, 0.8)
...
if __name__ == "__main__":
    try:
        if not os.path.exists(DATA_ROOT):
            print(f"\nСоздаю корневую папку {DATA_ROOT}...")
